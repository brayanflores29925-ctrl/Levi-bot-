  ...
  async execute(sock, m) {
    ...
    if (!audio) {
      return sock.sendMessage(chatId, {
        text: '🎧 Responde a un audio con /audiolento.'
      })
    }

    const id = Date.now()
    const entrada = `/tmp/al-${id}.ogg`
    const salida = `/tmp/al-${id}.mp3`
    ...
    try {
      ...
      await sock.sendMessage(chatId, {
        audio: buffer,
        mimetype: 'audio/mpeg',
        fileName: 'audio-lento.mp3'
      })

    } catch (error) {
      console.error('Error en /audiolento:', error)

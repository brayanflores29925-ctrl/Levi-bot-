  ...
  async execute(sock, m) {
    ...
    if (!audio) {
      return sock.sendMessage(chatId, {
        text: '🎧 Responde a un audio con /grave2.'
      })
    }

    const id = Date.now()
    const entrada = `/tmp/gr2-${id}.ogg`
    const salida = `/tmp/gr2-out-${id}.mp3`

    try {
      await sock.sendMessage(chatId, {
        text: '🔊 ⏳ Aplicando efecto grave 2...'
      })
      ...
      await execFileAsync('ffmpeg', [
        '-y',
        '-i', entrada,
        '-af', 'asetrate=44100*0.6,aresample=44100,atempo=1.666667',
        '-vn',
        '-c:a', 'libmp3lame',
        '-b:a', '128k',
        salida
      ...
      await sock.sendMessage(chatId, {
        audio: buffer,
        mimetype: 'audio/mpeg',
        fileName: 'audio-grave2.mp3'
      })

    } catch (error) {
      console.error('Error en /grave2:', error)
